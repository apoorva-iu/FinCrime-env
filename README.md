...
1. Create a new Space and choose the `Dockerfile` option.
2. Push this repository to the Space's GitHub repo or upload the files.
3. Set the required secrets/variables in the Space: `HF_TOKEN` (or `OPENAI_API_KEY`), `MODEL_NAME`, `API_BASE_URL`.
...
- **Scoring**: `F1(flagged_transactions) * 0.8 + risk_level_accuracy * 0.2`
...
2. What the example `curl` commands do:
...
3. Running the baseline inference (what those `export` lines mean):
...
4. Submission: GitHub + Hugging Face Spaces (recommended flow)
...
5. Pre-submission quick checklist (Windows)
...
OPENAI_API_KEY = (alternative to HF_TOKEN)
...
├── env.py — Environment logic + graders + reward function
├── models.py — Typed Pydantic models (Observation, Action, Reward)
├── inference.py — Baseline LLM agent script
├── cases.json — 15 synthetic fraud cases (easy/medium/hard)
